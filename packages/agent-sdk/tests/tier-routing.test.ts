...
describe('tier routing', () => {
  it('discovery is Haiku-only', () => {
    expect(allowedModelsForAgent('discovery')).toEqual([
  ...
  it('content allows Sonnet + Opus polish', () => {
    expect(allowedModelsForAgent('content')).toEqual([
      'claude-sonnet-4-6',
      'claude-opus-4-7',
    ]);
  });
  ...
  it('assertAgentModel throws TierRoutingError for forbidden combos', () => {
    ...
    expect(() => assertAgentModel('promotion', 'claude-haiku-4-5-20251001')).toThrow(TierRoutingError);
  });
