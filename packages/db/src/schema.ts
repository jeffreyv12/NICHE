...
export const nicheCandidates = pgTable(
  ...
  (t) => ({
    ...
    sourceSurfacedIdx: index("niche_candidates_source_surfaced_idx").on(t.source, t.surfacedAt),
  }),
);
...
export const nicheScores = pgTable(
  ...
  (t) => ({
    totalScoreCheck: check(
      "niche_scores_total_score_check",
      sql`${t.totalScore} between 0 and 100`,
    ),
